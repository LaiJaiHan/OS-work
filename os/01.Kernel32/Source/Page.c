...
void kInitializePageTables(void)
{
	...
	// new table
	pstNPTEntry = (NPTENTRY*)0x142000;
	dwMappingAddress = 0;
	for(i = 0; i < PAGE_MAXENTRYCOUNT; i++)
	{
		// 0x1ff000 -> RW(x)
		if(i == 511){
			//kSetPageEntryData(&(pstNPTEntry[i]), 0, dwMappingAddress,PAGE_FLAGS_P , 0);
			kSetPageEntryData(&(pstNPTEntry[i]), 0, dwMappingAddress,0 , 0);
		}
		else{
			kSetPageEntryData(&(pstNPTEntry[i]), 0, dwMappingAddress, PAGE_FLAGS_DEFAULT, 0);
		}
